<script lang="ts">
  ...
  // Use same pattern as IncidentFeed
  ...
  const detectionMatrix = $derived(() => {
    ...
    detectionMethods.forEach((method) => {
      timeSlots.forEach((timeSlot) => {
        ...
        const cellAnomalies = anomalies.filter((anomaly) => {
          // Check time slot
          const anomalyTime = new Date(
            anomaly.timestamp || anomaly.detection_timestamp
          );
          const hoursAgo =
            (now.getTime() - anomalyTime.getTime()) / (1000 * 60 * 60);
          const inTimeSlot =
            hoursAgo >= timeSlot.hours - 4 && hoursAgo < timeSlot.hours;
        ...
        const criticalCount = cellAnomalies.filter(
          (a) => a.severity_level === "CRITICAL" || a.final_anomaly_score >= 0.8
        ).length;
  ...
  const githubEvents = $derived(() => {
    const forcePushEvents = anomalies.filter(
      (a) =>
        a?.event_type === "PushEvent" && (a?.final_anomaly_score || 0) > 0.5
    );

    const workflowFailures = anomalies.filter(
      (a) =>
        a?.event_type === "WorkflowRunEvent" &&
        (a?.final_anomaly_score || 0) > 0.5
    );

    const highContentRisk = anomalies.filter(
      (a) => (a?.content_risk_score || 0) > 0.7
    );
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <div class="p-3 bg-red-900/20 border border-red-400/30 rounded-lg">
      ...
      <div class="text-lg font-mono text-red-400">
        {githubEvents?.forcePushEvents?.length || 0}
      </div>
    ...
    <div class="p-3 bg-yellow-900/20 border border-yellow-400/30 rounded-lg">
      ...
      <div class="text-lg font-mono text-yellow-400">
        {githubEvents?.workflowFailures?.length || 0}
      </div>
    ...
    <div class="p-3 bg-orange-900/20 border border-orange-400/30 rounded-lg">
      ...
      <div class="text-lg font-mono text-orange-400">
        {githubEvents?.highContentRisk?.length || 0}
      </div>
          ...
          <div class="space-y-2 max-h-48 overflow-y-auto">
            {#each selectedCell.anomalies.slice(0, 10) as anomaly}
              <div class="p-3 bg-gray-800/50 rounded border border-gray-600">
                <div class="flex items-center justify-between mb-2">
                  <span class="text-sm text-gray-100 font-mono"
                    >{anomaly.repository_name}</span
                  >
